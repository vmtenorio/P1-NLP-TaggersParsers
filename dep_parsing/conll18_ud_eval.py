...
def evaluate(gold_ud, system_ud, check_charseq=True, turn_ascii=False):
    class Score:
    ...
    if check_charseq and turn_ascii:
        gold_ud_characters = unidecode.unidecode("".join(gold_ud.characters))
        system_ud_characters = unidecode.unidecode("".join(system_ud.characters))
    else:
        gold_ud_characters = gold_ud.characters
        system_ud_characters = system_ud.characters
    if gold_ud_characters != system_ud_characters and check_charseq:
        index = 0
        while index < len(gold_ud_characters) and index < len(system_ud_characters) and \
                gold_ud_characters[index] == system_ud_characters[index]:
            index += 1

        raise UDError(
            "The concatenation of tokens in gold file and in system file differ!\n" +
            "First 20 differing characters in gold file: '{}' and system file: '{}'".format(
                "".join(map(_encode, gold_ud_characters[index:index + 20])),
                "".join(map(_encode, system_ud_characters[index:index + 20]))
            )
        )
